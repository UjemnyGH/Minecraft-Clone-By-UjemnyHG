...
void Text::Init()
{
	...
	auto* data = stbi_load("data/textures/characterAtlas.png", &x, &y, &bit, STBI_rgb_alpha);
...
void Text::SetText(std::string text, float r, float g, float b, float a)
{
	text_size = text.size();
...
void Text::TextAtlas(const char* text_atlas)
{
	glBindVertexArray(vao);
...
void Text::Pos(float x, float y, float z)
{
	psr[0][0] = x;
	psr[0][1] = y;
	psr[0][2] = z;
...
void Text::Scl(float x, float y, float z)
{
	psr[1][0] = x;
	psr[1][1] = y;
	psr[1][2] = z;
...
void Text::Rot(float x, float y, float z)
{
	psr[2][0] = x;
	psr[2][1] = y;
	psr[2][2] = z;
...
void Text::Draw(glm::mat4 proj, glm::mat4 view)
{
	glBindVertexArray(vao);
	glUseProgram(sh);
